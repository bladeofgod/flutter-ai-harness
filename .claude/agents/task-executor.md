...
2. 校验 `executor: task-executor`、`platforms` 与 `workKinds`；Android/iOS `native`、
   `bridge-adapter`、单平台原生门禁或跨 Runtime Wire/集成任务必须停止并返回正确 Executor。
3. 检查工作树并保护无关改动。
4. 确认阻塞项完成，目标文件与任务描述没有冲突。
5. 实现满足任务卡的最小完整行为。
6. 同批编写测试；除非任务卡明确拆分，不得留到后续。
7. 格式化触碰的 Dart 文件并运行受影响静态分析、聚焦测试和仓库 lint。
8. 共享契约或跨包改动需要扩大验证范围。
9. 由 `/execute-tasks` 调用时，验证命令统一通过证据采集器执行，不裸跑后再重复留证。
10. 不生成 UI Spec/Audit，不调用 App Operator；UI 自动化由人在任务流程之外独立安排。
