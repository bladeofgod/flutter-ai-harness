...
Tasks are planned in a new `docs/tasks/sprint-N/` directory and implemented one card at a time. A card marked `uiSpec: required` must first go through `/plan-spec`; a machine-validated `ready` Spec proceeds to static implementation audit, and only a passing audit can enter runtime execution. Human input is requested only when product decisions or external runtime state are missing. Implementation evidence and review findings are archived with the task, while only durable project knowledge belongs in `.claude/memories/`.
...
For runtime Flutter UI inspection, install Marionette with `make marionette-install`, start the Demo in Debug mode, and provide its `ws://.../ws` VM Service URI to the Agent. The operation sequence is connect, inspect or interact, then disconnect. Marionette operates the Flutter Widget Tree only and does not automate native system UI.
