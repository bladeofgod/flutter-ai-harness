...
Active task cards live directly under `docs/tasks/` and move to `docs/tasks/done/` when complete. Each filename uses a descriptive, globally unique lowercase kebab-case slug; no sprint number or task sequence is required. A card marked `uiSpec: required` must first go through `/plan-spec`; a machine-validated `ready` Spec proceeds to static implementation audit, and only a passing audit can enter runtime execution. Each declared platform then produces a structured App Operator report bound to the current implementation. Human input is requested only when product decisions or external runtime state are missing. Implementation evidence and review findings are archived with the task, while only durable project knowledge belongs in `.claude/memories/`.
...
The Harness baseline is complete. The Demo design source is registered and its foundation services are being planned; product UI and behavior are implemented through separate, traceable task cards after page-level Figma inputs are selected.
