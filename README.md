...
Tasks are planned in a new `docs/tasks/sprint-N/` directory and implemented one card at a time. A card marked `uiSpec: required` must first go through `/plan-spec`; only a reviewed `ready` Spec can proceed to static audit and runtime execution. Implementation evidence and review findings are archived with the task, while only durable project knowledge belongs in `.claude/memories/`.
