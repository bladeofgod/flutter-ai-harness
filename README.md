...
Active task cards live directly under `docs/tasks/` and move to `docs/tasks/done/` when complete. They may be created by a developer, an agent, a command, or another tool. Each filename uses a clear, descriptive, globally unique lowercase kebab-case slug; no sprint number, task sequence, producer prefix, or specific planning entry point is required. A card marked `uiSpec: required` must first go through `/plan-spec`; a machine-validated `ready` Spec proceeds to static implementation audit, and only a passing audit can enter runtime execution. Each declared platform then produces a structured App Operator report bound to the current implementation. Human input is requested only when product decisions or external runtime state are missing. Implementation evidence and review findings are archived with the task, while only durable project knowledge belongs in `.claude/memories/`.
