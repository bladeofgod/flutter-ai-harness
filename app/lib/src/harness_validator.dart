...
const _boundedEvidenceMaxFileBytes = 524288;
const _boundedEvidenceMaxFileLines = 4800;
const _boundedEvidenceMaxRecordBytes = 65536;
const _boundedEvidenceMaxRecordLines = 600;
const _evidenceUploadAction =
...
/// The immutable outcome of one complete Harness validation run.
final class HarnessValidationResult {
  HarnessValidationResult._(Iterable<String> diagnostics)
    : diagnostics = List.unmodifiable(diagnostics);

  /// Diagnostics in the order produced by the Harness validation phases.
  final List<String> diagnostics;

  /// Whether the repository passed every Harness validation phase.
  bool get isValid => diagnostics.isEmpty;
}

/// Validates [root] without printing or changing process exit state.
HarnessValidationResult validateHarness(Directory root) {
  final checker = _HarnessChecker(root.absolute);
  checker.run();
  return HarnessValidationResult._(checker.errors);
}
...
class _HarnessChecker {
  ...
  void _validateCiWorkflow() {
    ...
    final checkSteps = (jobs['check'] as YamlMap?)?['steps'];
    if (checkSteps is! YamlList ||
        !checkSteps.whereType<YamlMap>().any(
          (step) => (step['run'] as String?)?.contains('make check') ?? false,
        )) {
      errors.add('$relativePath 的 check Job 缺少 make check');
    }
    _validateCiBuildJob(
      jobs,
      relativePath,
    ...
    _validateCiEvidencePolicy(jobs, relativePath);
  }
  ...
  void _validateCiBuildJob(
    ...
    if (steps is! YamlList ||
        !steps.whereType<YamlMap>().any(
          (step) => (step['run'] as String?)?.contains(gateCommand) ?? false,
        )) {
      errors.add('$path 的 $name 缺少平台专项门禁：$gateCommand');
    }
  }

  void _validateCiEvidencePolicy(YamlMap jobs, String path) {
    final allSteps = jobs.values
        .whereType<YamlMap>()
        .map((job) => job['steps'])
        .whereType<YamlList>()
        .expand((steps) => steps.whereType<YamlMap>());
    if (!allSteps.any(
      (step) =>
          (step['run'] as String?)?.contains('capture-evidence.sh') ?? false,
    )) {
      return;
    }

    for (final jobName in const ['check', 'android-build', 'ios-build']) {
      final job = jobs[jobName];
      final steps = job is YamlMap ? job['steps'] : null;
      if (steps is! YamlList) {
        continue;
      }
      final typedSteps = steps.whereType<YamlMap>().toList(growable: false);
      final captureSteps = typedSteps.where(
        (step) =>
            (step['run'] as String?)?.contains('capture-evidence.sh') ?? false,
      );
      if (captureSteps.isEmpty ||
          captureSteps.any(
            (step) => !(step['run'] as String).contains('--artifact'),
          )) {
        errors.add('$path 的 $jobName 必须通过 capture-evidence 生成完整脱敏 Artifact');
      }
      final uploads = typedSteps
          .where((step) => step['uses'] == _evidenceUploadAction)
          .toList(growable: false);
      if (uploads.length != 1) {
        errors.add('$path 的 $jobName 必须且只能声明一个固定 SHA Artifact 上传步骤');
        continue;
      }
      final upload = uploads.single;
      final settings = upload['with'];
      if (upload['if'] != 'always()' ||
          settings is! YamlMap ||
          settings['retention-days'] != 14 ||
          settings['if-no-files-found'] != 'error' ||
          settings['include-hidden-files'] != false ||
          settings['path'] != r'${{ env.EVIDENCE_ARTIFACT }}') {
        errors.add('$path 的 $jobName Artifact 必须 always 上传、保留 14 天并在缺失时失败关闭');
      }
      final artifactName = settings is YamlMap ? settings['name'] : null;
      if (artifactName is! String ||
          !artifactName.contains(r'${{ github.job }}') ||
          !artifactName.contains(r'${{ github.run_attempt }}')) {
        errors.add('$path 的 $jobName Artifact 名称必须包含 Job 和 run attempt');
      }
    }
  }
  ...
  void _validateCompletedTaskArtifacts(
    ...
    if (!evidence.existsSync()) {
      ...
      final content = evidence.readAsStringSync();
      if (!RegExp(r'^Exit code: -?\d+$', multiLine: true).hasMatch(content)) {
        errors.add('$evidencePath 缺少命令退出码');
      }
      if (content.contains('Evidence format: bounded-v1')) {
        _validateBoundedEvidence(evidencePath, content);
      }
    }
  }

  void _validateBoundedEvidence(String path, String content) {
    if (utf8.encode(content).length > _boundedEvidenceMaxFileBytes ||
        '\n'.allMatches(content).length + 1 > _boundedEvidenceMaxFileLines) {
      errors.add('$path 超过有界证据整文件上限');
      return;
    }
    final records = content
        .split(RegExp(r'(?=^Evidence format: bounded-v1$)', multiLine: true))
        .where((record) => record.trim().isNotEmpty)
        .toList(growable: false);
    for (var index = 0; index < records.length; index += 1) {
      final record = records[index];
      final label = '$path 第 ${index + 1} 条命令';
      if (!record.startsWith('Evidence format: bounded-v1\n') ||
          utf8.encode(record).length > _boundedEvidenceMaxRecordBytes ||
          '\n'.allMatches(record).length + 1 > _boundedEvidenceMaxRecordLines) {
        errors.add('$label 的格式或单命令上限无效');
        continue;
      }
      final required = <RegExp>[
        RegExp(r'^## Command$', multiLine: true),
        RegExp(r'^## Tool Versions$', multiLine: true),
        RegExp(r'^## Result$', multiLine: true),
        RegExp(r'^Exit code: -?\d+$', multiLine: true),
        RegExp(r'^Output SHA-256: [0-9a-f]{64}$', multiLine: true),
        RegExp(r'^Original output bytes: \d+$', multiLine: true),
        RegExp(r'^Original output lines: \d+$', multiLine: true),
        RegExp(r'^Summary output bytes: \d+$', multiLine: true),
        RegExp(r'^Summary output lines: \d+$', multiLine: true),
        RegExp(r'^Truncated: (?:yes|no)$', multiLine: true),
        RegExp(
          r'^Selection: (?:success-stable-lines-v1|failure-root-context-v1)$',
          multiLine: true,
        ),
      ];
      if (required.any((pattern) => !pattern.hasMatch(record))) {
        errors.add('$label 缺少 bounded evidence 必填段或 metadata');
        continue;
      }
      ...
      if (outputMatch == null) {
        errors.add('$label 的 Output Summary 无效');
        continue;
      }
      final output = outputMatch.group(1)!;
      final declaredBytes = int.parse(
        RegExp(
          r'^Summary output bytes: (\d+)$',
          multiLine: true,
        ).firstMatch(record)!.group(1)!,
      );
      final declaredLines = int.parse(
        RegExp(
          r'^Summary output lines: (\d+)$',
          multiLine: true,
        ).firstMatch(record)!.group(1)!,
      );
      if (utf8.encode(output).length != declaredBytes ||
          '\n'.allMatches(output).length + 1 != declaredLines) {
        errors.add('$label 的 summary 计数与内容不一致');
      }
      if (record.contains('Truncated: yes') &&
          !output.contains('use the redacted CI artifact for full output')) {
        errors.add('$label 缺少显式 truncation marker');
      }
    }
  }
  ...
  void _validateMediaCaptureCrossRuntimeGolden(
    ...
    final generation = _capabilityObject(
      golden['generation'],
      '$path generation',
    );
    _validateCapabilityExactKeys(generation ?? const {}, const {
      'generatorVersion',
    ...
    final codeGeneration = _capabilityObject(
      wire['codeGeneration'],
      '$path Wire codeGeneration',
    );
    final generated = _capabilityObject(
      codeGeneration?['generated'],
      '$path Wire codeGeneration.generated',
    );
    int generatedCount(String key) => generated?[key] is List<Object?>
        ? (generated![key] as List<Object?>).length
        : -1;
    final descriptorDigest = generation?['normalizedDescriptorDigest'];
    if (generation?['generatorVersion'] !=
            codeGeneration?['generatorVersion'] ||
        generation?['wireVersion'] != wire['wireVersion'] ||
        generation?['methodCount'] != generatedCount('methodIds') ||
        generation?['eventCount'] != generatedCount('eventIds') ||
        generation?['resultTypeCount'] != generatedCount('resultTypeIds') ||
        generation?['failureTypeCount'] != generatedCount('failureTypeIds') ||
        generation?['errorCount'] != generatedCount('errorCodes') ||
        generation?['payloadDescriptorCount'] != generatedCount('payloadIds') ||
        generation?['fieldDescriptorCount'] != generatedCount('fieldIds') ||
        descriptorDigest is! String ||
        !RegExp(r'^[0-9a-f]{64}$').hasMatch(descriptorDigest)) {
      errors.add('$path generation 必须精确绑定当前生成 manifest 与 descriptor digest');
    }

    const contractPath = 'docs/bridge/contracts/media-capture.wire.json';
    final contractDigest = generation?['contractImplementationDigest'];
    if (contractDigest is! String ||
        !RegExp(r'^[0-9a-f]{64}$').hasMatch(contractDigest) ||
        calculateImplementationDigest(root, const [contractPath]) !=
            contractDigest) {
      errors.add('$path generation 必须绑定当前 Wire Contract 实现摘要');
    }

    final outputDigests = _capabilityObject(
      generation?['outputImplementationDigests'],
      '$path generation.outputImplementationDigests',
    );
    _validateCapabilityExactKeys(outputDigests ?? const {}, const {
      'dart',
      'android',
      'ios',
    }, '$path generation.outputImplementationDigests');
    const generatedOutputs = <String, String>{
      'dart':
          'app/packages/app_media_capture_bridge/lib/src/'
          'media_capture_wire.g.dart',
      'android':
      ...
      'ios':
          ...
          'app_media_capture_bridge/Sources/MediaCaptureBridgeCore/'
          'MediaCaptureWire.generated.swift',
    };
    for (final output in generatedOutputs.entries) {
      final file = _file(output.value);
      final expectedDigest = outputDigests?[output.key];
      if (FileSystemEntity.typeSync(file.path, followLinks: false) !=
              FileSystemEntityType.file ||
          expectedDigest is! String ||
          !RegExp(r'^[0-9a-f]{64}$').hasMatch(expectedDigest)) {
        errors.add('$path generation ${output.key} 必须绑定普通生成文件及小写 SHA-256');
        continue;
      }
      final source = file.readAsStringSync();
      if (!source.startsWith('// GENERATED CODE - DO NOT MODIFY BY HAND.\n') ||
          !source.contains('// Generator: media_capture_wire/1\n') ||
          !source.contains('// Source digest (SHA-256): $descriptorDigest\n') ||
          calculateImplementationDigest(root, [output.value]) !=
              expectedDigest) {
        errors.add('${output.value} 与当前 Contract 生成摘要或标记不一致');
      }
    }
    ...
    if (!_sameStringSet(boundConsumerPaths, expectedConsumerPaths)) {
      errors.add('$path consumerBindings 必须精确绑定三端消费者、生成测试与 iOS loader');
    }
  ...
  void _validateWireSchema(Map<String, Object?> schema, String path) {
    const expectedDigest =
        'bbbd0bb192b3f329b1736fed0570a5d057489386e462ce36779ccf7d11f541ab';
    ...
    const propertiesFields = {...fields, 'transferStore', 'codeGeneration'};
    final required = _capabilityStringSet(schema['required'], '$path required');
    final properties = _capabilityObject(
      schema['properties'],
      '$path properties',
    ...
    final definitions = _capabilityObject(schema[r'$defs'], '$path \$defs');
    final codeGeneration = definitions == null
        ? null
        : _capabilityObject(
            definitions['codeGeneration'],
            '$path codeGeneration',
          );
    final generatedReferences = definitions == null
        ? null
        : _capabilityObject(
            definitions['generatedContractReferences'],
            '$path generatedContractReferences',
          );
    final manualReference = definitions == null
        ? null
        : _capabilityObject(
            definitions['manualContractReference'],
            '$path manualContractReference',
          );
    if (codeGeneration?['type'] != 'object' ||
        codeGeneration?['additionalProperties'] != false ||
        generatedReferences?['type'] != 'object' ||
        generatedReferences?['additionalProperties'] != false ||
        manualReference?['type'] != 'object' ||
        manualReference?['additionalProperties'] != false) {
      errors.add('$path Base codeGeneration manifest 与引用必须是闭合 object');
    }
  ...
  void _validateMediaCaptureWire(
    ...
    _validateWireCodeGeneration(wire['codeGeneration'], wire, path);
  ...
  void _validateWireCodeGeneration(
    Object? value,
    Map<String, Object?> wire,
    String path,
  ) {
    final manifest = _capabilityObject(value, '$path codeGeneration');
    if (manifest == null) {
      return;
    }
    _validateCapabilityExactKeys(manifest, const {
      'generatorVersion',
    ...
    if (manifest['generatorVersion'] != 1 ||
        manifest['sourceDigestAlgorithm'] != 'sha256') {
      errors.add('$path codeGeneration 必须固定 generator Version 1 与 SHA-256');
    }

    const generatorPath = 'app/tool/generate_media_capture_wire.dart';
    const rendererPath = 'app/tool/src/media_capture_wire_generation.dart';
    final generator = _file(generatorPath);
    final renderer = _file(rendererPath);
    if (FileSystemEntity.typeSync(generator.path, followLinks: false) !=
            FileSystemEntityType.file ||
        !generator.readAsStringSync().contains(
          "import 'src/media_capture_wire_generation.dart';",
        )) {
      errors.add('$path codeGeneration 缺少普通文件形式的 generator 入口');
    }
    if (FileSystemEntity.typeSync(renderer.path, followLinks: false) !=
            FileSystemEntityType.file ||
        const [
          '_renderDart(',
          '_renderKotlin(',
          '_renderSwift(',
        ].any((marker) => !renderer.readAsStringSync().contains(marker))) {
      errors.add('$path codeGeneration 缺少 Dart、Kotlin 或 Swift renderer 实现');
    }

    final outputs = _wireObjectsByWireKey(
      manifest['outputs'],
      'runtime',
      '$path codeGeneration.outputs',
    );
    const expectedOutputs = <String, Map<String, Object?>>{
      'dart': <String, Object?>{
        'runtime': 'dart',
        'language': 'dart',
        'path':
            'app/packages/app_media_capture_bridge/lib/src/'
            'media_capture_wire.g.dart',
        'identifierStyle': 'lower_camel_case',
        'typeStyle': 'upper_camel_case',
      },
    ...
    if (!_sameStringSet(outputs.keys.toSet(), expectedOutputs.keys.toSet())) {
      errors.add('$path codeGeneration 必须逐一登记 Dart、Android 与 iOS 输出');
    }
    for (final entry in expectedOutputs.entries) {
      final actual = outputs[entry.key];
      if (actual == null ||
          actual.length != entry.value.length ||
          entry.value.entries.any(
            (property) => actual[property.key] != property.value,
          )) {
        errors.add('$path codeGeneration ${entry.key} 输出路径或名称映射漂移');
      }
    }

    final generated = _capabilityObject(
      manifest['generated'],
      '$path codeGeneration.generated',
    );
    if (generated == null) {
      return;
    }
    _validateCapabilityExactKeys(generated, const {
      'wireVersionPointer',
      'channelIds',
      'methodIds',
      'eventIds',
    ...
    if (generated['wireVersionPointer'] != '/wireVersion') {
      errors.add('$path codeGeneration 必须引用当前 wireVersion');
    }

    Set<String> ids(Object? source, String key, String label) =>
        _wireObjectsById(source, key, label).keys.toSet();

    final methods = _wireObjectsById(wire['methods'], 'id', '$path methods');
    final resultTypes = <String>{};
    for (final method in methods.values) {
      final resultType = method['resultType'];
      if (resultType is String) {
        resultTypes.add(resultType);
      }
      final outcomes = method.containsKey('terminalOutcomes')
          ? _capabilityObjectList(
              method['terminalOutcomes'],
              '$path method terminalOutcomes',
            )
          : null;
      if (outcomes != null) {
        resultTypes.addAll(
          outcomes.map((outcome) => outcome['resultType']).whereType<String>(),
        );
      }
    }
    final payloadIds = <String>{
      ...ids(wire['payloads'], 'id', '$path payloads'),
      ...ids(wire['failurePayloads'], 'id', '$path failurePayloads'),
    };
    final expectedReferences = <String, Set<String>>{
      'channelIds': ids(wire['channels'], 'id', '$path channels'),
      'methodIds': methods.keys.toSet(),
      'eventIds': ids(wire['events'], 'id', '$path events'),
      'resultTypeIds': resultTypes,
      'failureTypeIds': ids(wire['asyncFailures'], 'id', '$path asyncFailures'),
      'errorCodes': ids(wire['errors'], 'code', '$path errors'),
      'errorDetailKeys': _wireObjectsByWireKey(
    ...
    for (final reference in expectedReferences.entries) {
      final actual = _capabilityStringSet(
        generated[reference.key],
        '$path codeGeneration.generated.${reference.key}',
      );
      if (actual == null || !_sameStringSet(actual, reference.value)) {
        errors.add(
          '$path codeGeneration.generated.${reference.key} '
          '必须精确引用当前 Contract ID 集',
        );
      }
    }
    final errorProperties = _capabilityStringSet(
      generated['errorPropertyNames'],
      '$path codeGeneration.generated.errorPropertyNames',
    );
    if (errorProperties == null ||
        !_sameStringSet(errorProperties, const {
          'source',
          'capabilityFailureId',
          'recoverable',
          'terminal',
      ...
      errors.add('$path codeGeneration 必须生成全部 error 属性');
    }
    const expectedEnvelopePointers = <String>{
    ...
    const expectedTransportPointers = <String>{
      '/transportConstraints/signedInteger',
      '/transportConstraints/opaqueHandles',
    };
    for (final entry in <String, Set<String>>{
      'envelopePointers': expectedEnvelopePointers,
      'transportConstraintPointers': expectedTransportPointers,
    }.entries) {
      final actual = _capabilityStringSet(
        generated[entry.key],
        '$path codeGeneration.generated.${entry.key}',
      );
      if (actual == null || !_sameStringSet(actual, entry.value)) {
        errors.add('$path codeGeneration.generated.${entry.key} 不完整');
      }
    }

    final manualOnly = _wireObjectsById(
      manifest['manualOnly'],
      'id',
      '$path codeGeneration.manualOnly',
    );
    const expectedManualPointers = <String, Set<String>>{
      'capability_model_mapping': {'/capability', '/coverage'},
      'method_dispatch': {'/methods', '/failureDelivery'},
      'async_completion_and_lifecycle': {
        '/lifecycle/linearizationPolicy',
        '/lifecycle/resultCompletionPolicies',
        '/lifecycle/lateResultPolicies',
      },
      'listener_and_owner_generation': {
        '/lifecycle/listenerPolicy',
        '/lifecycle/boundaries',
      },
      'platform_thread_dispatch': {'/lifecycle/callbackThread', '/platform'},
      'resource_and_transfer_ownership': {
        '/lifecycle/resourceAdoptionPolicies',
        '/transferStore',
      },
      'file_uri_and_native_sdk_behavior': {
        '/transferStore/fileUriPolicy',
        '/coverage/nativeArtifacts',
      },
      'cross_field_validation': {'/fieldMappings'},
      'logging_and_redaction': {'/security', '/transferStore/redaction'},
    };
    if (!_sameStringSet(
      manualOnly.keys.toSet(),
      expectedManualPointers.keys.toSet(),
    )) {
      errors.add('$path codeGeneration.manualOnly 必须覆盖全部不可生成行为');
    }
    for (final entry in manualOnly.entries) {
      _validateCapabilityExactKeys(entry.value, const {
        'id',
        'contractPointers',
        'reason',
      }, '$path codeGeneration.manualOnly.${entry.key}');
      final pointers = _capabilityStringSet(
        entry.value['contractPointers'],
        '$path codeGeneration.manualOnly.${entry.key}.contractPointers',
      );
      final expectedPointers = expectedManualPointers[entry.key];
      if (pointers == null ||
          expectedPointers == null ||
          !_sameStringSet(pointers, expectedPointers)) {
        errors.add(
          '$path codeGeneration.manualOnly.${entry.key} 必须精确引用批准的 coverage pointer',
        );
      } else {
        for (final pointer in pointers) {
          if (!_wireContractPointerExists(wire, pointer)) {
            errors.add('$path codeGeneration manual-only pointer 不存在：$pointer');
          }
        }
      }
      if (!_wireNonEmpty(entry.value['reason'])) {
        errors.add('$path codeGeneration.manualOnly.${entry.key} 缺少原因');
      }
    }
  }

  bool _wireContractPointerExists(Map<String, Object?> wire, String pointer) {
