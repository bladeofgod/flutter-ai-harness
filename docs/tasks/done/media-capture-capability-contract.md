...
- 现有订单评价设计与页面已经存在，但画板不包含 Media Capture。用户已批准 Demo V1 采用微信式拍摄器交互作为设计方向：点击拍照、长按录像、缩放、切换镜头、闪光灯、对焦、拍后预览、重拍、确认和取消；不得复制微信品牌资源或逐像素照搬，订单评价入口和附件状态继续复用 Shoppe 的 Token 与页面语言。
...
- 不在本 Contract 任务中实现或细化 UI 布局、动画参数和品牌样式；后续 Native UI 任务按已批准的微信式交互方向与 Shoppe Token 落地，不再等待专用 Figma。
...
本任务不连接 Camera、设备、Figma 或 Marionette。专用 Figma 不再是后续实现前置条件：Native UI 使用已批准的微信式交互方向，Shoppe 媒体入口、预览结果和附件状态使用现有设计体系；任何超出 V1 范围的新交互仍需重新确认。
...
- [实现 Review](../../reviews/execute-media-capture-capability-contract.md)
- [Security Review](../../reviews/security-media-capture-capability-contract.md)
- [测试证据](../../reviews/test-evidence/media-capture-capability-contract.log)
