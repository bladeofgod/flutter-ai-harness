...
- 已有订单评价设计和实现，以及用户批准的微信式拍摄器交互方向与 Shoppe Token 接入边界。
...
- 不在本 Gate 中实现 Native UI、媒体入口、附件状态或 Shoppe 视觉接入；这些属于必须生成并单独执行的第二阶段任务。
...
3. 第二阶段至少拆分以下所有权，具体 slug 由实际产物命名：
   ...
   - Android 微信式 Native Capture UI：`android-engineer`、`platforms: [android]`、`workKinds: [native]`。
   - iOS 微信式 Native Capture UI：`ios-engineer`、`platforms: [ios]`、`workKinds: [native]`。
   - Dart Client：`task-executor`、`platforms: [flutter]`、`workKinds: [dart-client]`。
   - Shoppe 订单评价媒体入口、结果预览与附件状态：`task-executor`、`platforms: [flutter]`、`workKinds: [flutter]`。
...
4. Core 只依赖 Capability Contract；Dart Client 和两端 Bridge Adapter 依赖 Wire Contract；平台 Adapter 与 Native UI 额外依赖对应 Core；Shoppe 接入依赖 Dart Client 和可调用的跨端集成；最终集成依赖 Dart Client、两端 Adapter、两端 Native UI 和两端平台门禁。任务图必须无环并允许两端平台工作在没有共享写入时并行。
...
7. 专用 Figma 不再是第二阶段阻塞项。Native UI 按已批准的微信式交互实现，但不得复制微信品牌资源或逐像素照搬；Shoppe 入口、结果预览和附件状态使用现有 Token 与页面语言。超出已批准 V1 的新交互必须停止并重新确认。
8. 生成后运行 Harness 门禁，并由没有参与任务生成的独立 Reviewer 检查元数据、Executor、DAG、共享写入、Capability/Wire 依赖和已批准设计边界。存在 P0/P1 时本 Gate 不得归档，且任何第二阶段任务不得开始执行。
...
本任务只验证 Bootstrap 产物并生成、审查第二阶段任务卡，不需要 Android SDK、Xcode、设备、Figma MCP 或 Marionette。微信式交互方向已足以规划 Demo V1，不要求补充专用 Figma；任何真实平台能力、视觉结果和 Host 构建证据必须由第二阶段对应任务产生。

## 执行结果

- [独立 Review](../../reviews/execute-native-harness-bootstrap-gate.md)
- [测试证据](../../reviews/test-evidence/native-harness-bootstrap-gate.log)
- Bootstrap 与第二阶段任务图已完成；Media Capture 实现尚未开始。
