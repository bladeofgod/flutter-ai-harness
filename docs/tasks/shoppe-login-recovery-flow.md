...
- `shoppe-auth-local-foundation` 的 Fixture Auth API 与 Service
- `shoppe-registration-flow` 建立的 Auth 私有组件、资源和 Route 结构
...
2. 创建 Login Controller，通过构造函数接收 `AuthApi` 与壳工程回调。Controller 管理 Email 查询、已识别 User、8 位密码、提交中、错误状态和 Recovery 跳转，不读取 AuthService/UserService。Controller 由单一 `LoginFlowScope`（`ShellRoute` 或等价父级装配 Scope）持有，在 Login/Password/Recovery 子 Route 间复用，只在离开整个登录流程时销毁并清除 Secret、订阅和输入状态。
...
5. 密码必须恰好 8 位；按设计上下文的 Demo 默认，输入满 8 位或键盘 `Go` 时提交。正确结果回调把 `AuthResult` 交给壳工程 `AuthStateCoordinator`，由它原子提交匹配的 User/Session 并只发送一次共享通知，再 `go('/profile')`；错误清除真实输入，显示 8 个 `#EC4E4E` 红点和 `Forgot your password?`，下一次输入恢复普通圆点。
...
9. 在 `feature_auth/routes.dart` 汇总 login/password/recovery Route，并使用稳定父级 Key 保持同一 `LoginFlowScope`；子 Route 不得各自创建 Controller。壳工程把 Welcome 次操作接到 `/auth/login`，并提供 authenticated/cancel 回调。不得从 Welcome 或 Profile import Auth 内部页面。
...
- Route：Welcome 次操作进入 Login；Email -> Password；Forgot -> Recovery；缺少已识别 User 时 direct Password/Recovery 回 Login；各级 Cancel/Back；成功替换到 Profile；已登录 Auth Redirect；验证 Scope 不重复创建。
...
- 页面/Controller 只依赖 AuthApi 与 Domain 类型，壳工程独占 Session/User 更新和根导航。
