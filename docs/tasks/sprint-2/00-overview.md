...
本 Sprint 启动 `AI Harness Shoppe` Demo 的真实迭代。`ApiClient`、`AuthService` 和 `UserService` 已批准纳入 Demo 架构；首张任务先固化三者的 Demo 化职责、数据流和实现门槛，防止在具体页面和业务流程确定前复制参考工程的网络、认证、存储或数据库复杂度。
...
- 参考工程分析结论：保留接口自治、构造注入、全局 Service 生命周期和分层初始化模式，不复制参考工程业务代码、协议、环境值或认证编排。
...
1. 完成基础 Service 候选能力的采用、延后和禁止迁移矩阵。
...
- 参考工程的三个类名可以保留架构表达，但具体实现绑定私有 Endpoint、Proto 信封、Token 刷新、IM、账号切换和平台 SDK，不能直接复制。
